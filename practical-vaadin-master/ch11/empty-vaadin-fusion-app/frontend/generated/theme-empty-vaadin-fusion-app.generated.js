...
import stylesCss from 'themes/empty-vaadin-fusion-app/styles.css';
...
window.Vaadin['_vaadintheme_empty-vaadin-fusion-app_globalCss'] = window.Vaadin['_vaadintheme_empty-vaadin-fusion-app_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_empty-vaadin-fusion-app_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_empty-vaadin-fusion-app_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_empty-vaadin-fusion-app_globalCss'].push(target);
  }
  if (!document['_vaadintheme_empty-vaadin-fusion-app_componentCss']) {
    
    document['_vaadintheme_empty-vaadin-fusion-app_componentCss'] = true;
  }
